...
// ── SUGGESTION ENGINE ──────────────────────────────────
// Find the best alternative for an esgotada pilgrimage
SJ_DATA.getSuggested = function(p) {
  ...
  // 2. Closest upcoming
  var dated = available.filter(function(x) {
    var d = SJ_DATA.getDate(x);
    return d && d >= new Date();
  });
...
// ── SHARED buildCard() ─────────────────────────────────
// Used by index, peregrinacoes, cancao-nova pages
SJ_DATA.buildCard = function(p) {
  ...
  if (isCancelado) {
    ...
    ctaHtml = '<button class="btn btn-green" style="width:100%;justify-content:center;opacity:0.5;cursor:not-allowed;" disabled>Ver Detalhes — Esgotada</button>'
            ...
            +   '<button class="btn-wl-submit" onclick="submitWaitlist(\'' + p.id + '\',\'' + p.title + '\')">Confirmar interesse</button>'
            + '</div>';
...
// ── WAITLIST HELPERS (injected once into any page that uses buildCard) ─────
